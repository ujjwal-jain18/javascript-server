...
export default (config: object) => (req: Request, res: Response, next: NextFunction): void => {
    ...
    validationKeys.forEach(validateKey => {
        ...
        if ( Object.keys(validationRules).includes('in') ) {
            ...
            } else if (validationRules.in.includes('query') && ( Object.keys(dataFromQuery).includes('skip') === false) ) {
                        req.query = {
                            ... req.query,
                            skip: '0',
                        };
            } else if (validationRules.in.includes('query') && ( Object.keys(dataFromQuery).includes('limit') === false) ) {
                        req.query = {
                            ... req.query,
                            limit: '10',
                };
